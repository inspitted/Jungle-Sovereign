...
let gameState = {
    ...
    exp: 0,
    stage: 1,
    weaponsOwned: [0], // IDs of owned weapons
    equippedWeapon: 0,
    petsOwned: [], // Array of objects: { id (unique), tier, emoji }
    equippedPets: [], // Array of unique pet IDs
    enemyMaxHP: 150,
    enemyCurrentHP: 150,
};
...
function loadGame() {
    ...
    recalculateEnemyHP(false); // Do not alter current ratio on pure load
    updateUI();
    renderArmory();
    renderGacha();
    renderInventory();
...
function getWeaponPowerFactor(power) {
    // A balancing factor: higher power slightly lowers Max HP to reward gearing up
    // using logarithmic scaling to prevent zero-HP breakage
...
function recalculateEnemyHP(isWeaponSwitch = false) {
    ...
    if (isWeaponSwitch) {
        // Rescale current HP based on the new max
...
function handleAttack(event) {
    ...
    // Pet Ability Calculations (10% chance per pet)
    ...
    gameState.clicks += damage; // Gain currency based on DMG dealt
...
function checkEnemyDeath() {
    if (gameState.enemyCurrentHP <= 0) {
        // Gain EXP and Next Stage
        const expGain = gameState.stage * 10;
        gameState.exp += expGain;
        gameState.stage++;
        
        // Level up logic (Exp required = Level * 50)
        ...
        gameState.enemyCurrentHP = gameState.enemyMaxHP;
        changeEnemySprite();
        renderArmory(); // Update unlock buttons
    }
}

function changeEnemySprite() {
    const sprites = ['🦠','🕷️','🦂','🦇','🐗','🐍','🐆','🐊','🦍','🦖'];
    const idx = (gameState.stage - 1) % sprites.length;
    enemySprite.textContent = sprites[idx];
}
...
function spawnFloatingText(x, y, damage, isCrit, procText) {
    ...
    // Slight randomization on position
...
setInterval(() => {
    ...
    if (cps > 0) {
        ...
        // Apply passive damage as well to prevent soft-locks
...
function updateUI() {
    ...
    // Re-check button disable states dynamically without full re-render
...
function renderArmory() {
    ...
    WEAPONS.forEach(weapon => {
        if (weapon.id === 0) return; // Skip bare hands
...
window.equipWeapon = function(id) {
    ...
    recalculateEnemyHP(true); // Adjust HP dynamically on switch
    renderArmory();
    updateUI();
};
