...
const WEAPONS = [
    ...
    { id: 'w2', name: 'Rusty Machete', icon: '🗡️', req: 3, cost: 500, power: 10 },
    { id: 'w3', name: 'Poison Blowgun', icon: '🏹', req: 7, cost: 4000, power: 50 },
    { id: 'w4', name: 'Hunter\'s Shotgun', icon: '🔫', req: 12, cost: 25000, power: 250 },
    { id: 'w5', name: 'Ranger Blade', icon: '⚔️', req: 20, cost: 150000, power: 1500 },
...
function addXP(amount) {
    ...
    // Looping diaktifkan jika player dapet XP gede melompati level requirement
    ...
    // FIX BUG LEVEL REQ: Render armory ditaruh di updateUI agar sinkron secara realtime!
}
...
function setupEnemies() {
    ...
    if (userState.equippedWeapon) {
        ...
        if (weapon) {
            ...
            if (power <= 2) scalingFactor = 1.1;  
            else if (power <= 10) scalingFactor = 3.5;  
            else if (power <= 50) scalingFactor = 12.0; 
            else if (power <= 250) scalingFactor = 45.0;
            else if (power <= 1500) scalingFactor = 220.0;
            else scalingFactor = 1100.0; 
...
function renderArmory() {
    ...
    WEAPONS.forEach(w => {
        ...
        if (isEquipped) {
            ...
            // Perbaikan tampilan tombol gembok jika level tidak memenuhi
            if (userState.level < w.req) {
                btnHtml = `<button class="shop-btn" disabled style="background:#541616;">🔒 Lvl ${w.req}</button>`;
            } else {
...
// NEW FEATURE: Render gacha egg include dropdown view list content pet info inside
function renderEggs() {
    ...
    EGGS.forEach((egg, idx) => {
        ...
        // Membikin HTML list isi pet drop rate secara loop dinamis
        let poolHtml = '';
        egg.pool.forEach(p => {
            let skillText = p.skill ? ` | Skill: ${p.name.includes('Eagle') ? 'Wind Slash' : p.skill.split(' ')[0]}` : '';
            poolHtml += `
                <div class="pet-pool-row">
                    <span class="pool-name">${p.name} (${p.type})</span>
                    <span class="pool-stats">+${p.cps} ⚡/s${skillText}</span>
                    <span class="pool-chance">${Math.floor(p.rate * 100)}%</span>
                </div>
            `;
        });
            ...
            <div class="shop-card">
                <div class="card-main-row">
                    ...
                    <div style="display:flex; flex-direction:column; gap:5px;">
                        ...
                        <button class="shop-btn view-btn" onclick="toggleEggView('${egg.id}')">View Contents 👁️</button>
                    </div>
                </div>
                <!-- Box Dropdown info list isi pet -->
                <div id="view-${egg.id}" class="egg-contents-box">
                    <div style="font-size:0.7rem; font-weight:bold; color:#ffcc00; text-align:center; border-bottom:1px solid #386f41; padding-bottom:3px; margin-bottom:4px;">CAPSULE DROP RATES INFO</div>
                    ${poolHtml}
                </div>
            </div>
        `;
...
// Fungsi toggle buka tutup list pet egg
function toggleEggView(eggId) {
    const box = document.getElementById(`view-${eggId}`);
    if (box) {
...
// --- UI REFRESH ---
function updateUI() {
    // FIX BUG: Panggil renderArmory di awal updateUI agar sinkronisasi tombol lock/unlock ganti level berjalan realtime!
    renderArmory();
    ...
    document.querySelectorAll('#egg-list .shop-btn:not(.view-btn)').forEach((btn, i) => {
